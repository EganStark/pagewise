...
begin
  ...
  conflict_columns := case table_name
    ...
    when 'user_settings' then array['user_id'] else null end;
  ...
  select jsonb_agg(value || jsonb_build_object('user_id', auth.uid())) into sanitized_rows from jsonb_array_elements(p_rows);
  select string_agg(format('%I', attname), ', ' order by attnum),
         string_agg(format('%1$I = excluded.%1$I', attname), ', ' order by attnum) filter (where not (attname = any(conflict_columns)) and attname <> 'user_id')
    into column_list, update_list
    from pg_attribute where attrelid = p_table and attnum > 0 and not attisdropped and attgenerated = '';
  select string_agg(format('%I', value), ', '), string_agg(format('t.%1$I = r.%1$I', value), ' and ')
